  ...
  return (
    <div>
      <div className="page-header">
        <div>
          <h1><PeopleIcon style={{ verticalAlign: "middle", marginRight: 8 }} />User Management</h1>
          <p>{users.length} users in clinic</p>
        </div>
        <button className="btn btn-primary" onClick={() => setShowModal(true)}>
          <PersonAddIcon fontSize="small" /> Create User
        </button>
      </div>
        ...
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              ...
              <button className="modal-close" onClick={() => setShowModal(false)}><CloseIcon fontSize="small" /></button>
            </div>

            {formError && <div className="alert alert-error"><WarningAmberIcon fontSize="small" /> {formError}</div>}
